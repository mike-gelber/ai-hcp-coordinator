...
function getPrescribingInsight(profile: DemoHcpProfile): string {
  const { brandVsGeneric, prescribingVolume, topTherapeuticArea } =
    profile.prescribingProfile;
    ...
    msg(
      ...
      `Got it — incorporating those edits now. Here's the updated status:\n\n` +
        ...
        `- Personalization score: ${
          profile.digitalPresence.isKol ? "9.2/10" : profile.prescribingProfile.prescribingVolume === "high" ? "8.7/10" : "8.1/10"
        }\n\n` +
